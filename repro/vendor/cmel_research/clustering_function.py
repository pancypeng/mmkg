...
def _parse_cluster_label(response, valid_labels, fallback):
    """解析模型返回的数值标签；无效输出退回语义最近邻，而非任意簇 1。"""
    match = re.search(r"(?<!\d)-?\d+(?!\d)", response or "")
    if match:
...
"""
        ...
        # 调用 LLM 进行分类
        _, nearest_idx = NearestNeighbors(n_neighbors=1, metric="cosine").fit(
            embeddings
        ).kneighbors(input_embedding)
        fallback_label = labels[nearest_idx[0][0]]
        response = get_llm_response(cur_prompt=prompt_user, system_content=prompt_system)
        target_label = _parse_cluster_label(response, set(labels), fallback_label)
...
def get_possible_entities_text_clustering(
    ...
    dense_top_k = max(1, int(os.environ.get("CMEL_DENSE_CANDIDATE_TOP_K", "5")))
    ...
    if classify_method == "knn":
        ...
        for image_entity, input_embedding in zip(filtered_image_entity_list, input_embeddings):
            ...
            image_entity_with_labels.append({
                ...
                "dense_candidates": [nearby_text_entity_list[idx] for idx in
                                     np.argsort(cosine_similarity(
                                         [input_embedding], embeddings)[0])[::-1][:dense_top_k]],
            })
    elif classify_method == "llm":
        clusters = {}
        for label in set(labels):
...
def judge_text_entity_alignment_clustering(image_entity_with_labels, text_clustering_results):
    ...
    prompt_version = os.environ.get("CMEL_ALIGNMENT_PROMPT_VERSION", "paper_v1")
    if prompt_version == "hybrid_verify_v3":
        return _judge_text_entity_alignment_hybrid(image_entity_with_labels,
                                                    text_clustering_results)
    strict_guidance = ""
    if prompt_version == "strict_v2":
        strict_guidance = """
CMEL strict alignment rules:
- A shared cluster is only a candidate pool. Do not merge entities merely because they are
  related, co-mentioned, or share the same cluster label.
- For each visual entity, choose the smallest sufficient set of text entities that names or
  anchors what is visible. An unnamed object may use its best location/event anchor, but do not
  add secondary locations, causes, authors, methods, or related objects just for extra context.
- Put multiple text entity names in one source_text_entities list only when the names are explicit
  aliases/coreferential mentions of the same target. Do not enumerate every relevant cluster member.
- Normally create a separate record for each independently linkable image entity. Combine multiple
  image entities only when they jointly realize one plural/collective text entity or clearly denote
  the same target.
- Copy source entity names exactly from the inputs. Do not invent source names. If a link is
  uncertain, omit it instead of making a broad merge.
"""
...
def _judge_text_entity_alignment_hybrid(image_entity_with_labels, text_clustering_results):
    """Verify one visual entity at a time using SpecLink-cluster ∪ dense candidates."""
    clusters = {cluster["label"]: cluster["entities"] for cluster in text_clustering_results}
    outputs = []
    for image_entity in image_entity_with_labels:
        candidates = []
        seen = set()
        for candidate in [*clusters.get(image_entity["label"], []),
                          *image_entity.get("dense_candidates", [])]:
            name = candidate.get("entity_name")
            if name and name not in seen:
                seen.add(name)
                candidates.append({
                    "entity_name": name,
                    "entity_type": candidate.get("entity_type", "N/A"),
                    "description": candidate.get("description", ""),
                })
        allowed_names = {candidate["entity_name"] for candidate in candidates}
        canonical_names = {}
        for name in allowed_names:
            key = str(name).strip().strip("\"'").casefold()
            canonical_names.setdefault(key, []).append(name)
        image_payload = {
            "entity_name": image_entity["entity_name"],
            "entity_type": image_entity["entity_type"],
            "description": image_entity["description"],
        }
        prompt_user = f"""
Align exactly one image entity to zero or one semantic target represented by the candidate text
entities. Candidates come from the union of a SpecLink graph cluster and dense embedding Top-K.
...
Rules:
1. Examine the image entity even if the match is contextual: when an unnamed visual object has no
   literal textual name, select its single best location, event, person, or concept anchor.
2. Return no more than one merged record. Use source_image_entities exactly
   ["{image_entity['entity_name']}"]. Never add another image entity.
3. source_text_entities must be the smallest sufficient semantic target set. Include every candidate
   name that is an explicit alias/coreferential name of that chosen target, but exclude merely related
   locations, causes, authors, methods, components, or co-mentioned objects.
4. Copy source names exactly from Candidate text entities; do not invent or normalize them.
5. If no candidate plausibly grounds the visual entity, return [].

Return only a JSON list. Each item must contain entity_name, entity_type, description,
source_image_entities, and source_text_entities.
"""
        prompt_system = (
            "You are a precise cross-modal entity-linking verifier. Preserve recall while avoiding "
            "broad many-to-many merges, and output strict JSON only."
        )
        response = get_llm_response(cur_prompt=prompt_user, system_content=prompt_system)
        normalized = normalize_to_json_list(response)
        for item in normalized[:1]:
            if not isinstance(item, dict):
                continue
            text_names = []
            for name in item.get("source_text_entities") or []:
                resolved = name if name in allowed_names else None
                if resolved is None:
                    matches = canonical_names.get(str(name).strip().strip("\"'").casefold(), [])
                    resolved = matches[0] if len(matches) == 1 else None
                if resolved is not None and resolved not in text_names:
                    text_names.append(resolved)
            if not text_names:
                continue
            outputs.append({
                "entity_name": item.get("entity_name", text_names[0]),
                "entity_type": item.get("entity_type", "N/A"),
                "description": item.get("description", ""),
                "source_image_entities": [image_entity["entity_name"]],
                "source_text_entities": text_names,
            })
    return outputs
