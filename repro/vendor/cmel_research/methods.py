...
def judge_image_entity_alignment(image_entity_name, image_entity_description, possible_image_matched_entities, nearby_chunks):
    ...
    matched_entity_name = get_llm_response(image_entity_judgement_user, PROMPTS["image_entity_judgement_system"]).strip()
    try:
        decoded = json.loads(matched_entity_name)
        if isinstance(decoded, str):
            matched_entity_name = decoded
    except json.JSONDecodeError:
        matched_entity_name = matched_entity_name.strip('"\'')
